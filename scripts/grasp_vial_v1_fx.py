...
class VialGrasper:
    def __init__(self):
        ...
        self.robot = Robot("172.16.0.2")  # Change IP address 
    ...
    def move_to_neutral(self):
        joint_position = [0.0, -0.785, 0.0, -2.356, 0.0, 1.57, 0.785]
        self.robot.move_to_joint_position(joint_position)

    def move_to_check(self):
        joint_position = [0.19723233, -0.5032747, 0.39010309, -2.23760213, 0.12057556, 1.74502069, 1.25511978]
        self.robot.move_to_joint_position(joint_position)
    ...
    def pose_callback(self, pose_msg):
        ...
        print('ee_pose()', self.robot.current_pose())
        print('self.E2Q(pose_msg)', self.E2Q(pose_msg))
        target_pose_0 = self.execute(pose_matrices)

        pose_affine = Affine(
            x=target_pose_0.pose.position.x,
            y=target_pose_0.pose.position.y,
            z=target_pose_0.pose.position.z,
            a=target_pose_0.pose.orientation.x,
            b=target_pose_0.pose.orientation.y,
            c=target_pose_0.pose.orientation.z
        )
        
        self.robot.move_straight_to(pose_affine)
