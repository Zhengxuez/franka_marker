...
def parse_arguments():
    ...
    parser.add_argument('--mode', choices=['record_waypoint', 'calculate_Tmarker', 'execute_saved_waypoints', 'save_eye_hand', 'save_Tbase_to_ee', 'save_current_Tcamera_to_marker', 'save_Tcamera_to_marker_1'], required=True,
    ...
    def on_press(self, key):
        ...
        if key == keyboard.Key.enter:
            ...
            self.recorded_poses.append({
                'position': {
                    'x': ee_pose.translation().x(),
                    'y': ee_pose.translation().y(),
                    'z': ee_pose.translation().z()
                },
                'orientation': {
                    'w': ee_pose.rotation().w(),
                    'x': ee_pose.rotation().x(),
                    'y': ee_pose.rotation().y(),
                    'z': ee_pose.rotation().z()
                }
            })
            print(f"Recorded Pose #{len(self.recorded_poses)}: Position: {ee_pose.translation()}, Orientation: {ee_pose.rotation()}")
